...
<?php } ?>
<div class="cel_album"><?php echo $libitem->f_link; ?></div>
<div class="cel_add">
    <span class="cel_item_add">
        <?php if ($show_playlist_add) { ?>
            ...
            <a id="<?php echo 'add_playlist_'.$libitem->id ?>" onclick="showPlaylisdivialog(event, 'album', '<?php if (!count($libitem->album_suite)) { echo $libitem->id; } else { echo implode(',', $libitem->album_suite); } ?>')">
                <?php echo UI::get_icon('playlist_add', T_('Add to existing playlist')); ?>
            </a>
        <?php } ?>
    </span>
</div>
<div class="cel_artist"><?php echo (!empty($libitem->f_album_artist_link) ? $libitem->f_album_artist_link : $libitem->f_artist_link); ?></div>
<div class="cel_songs"><?php echo $libitem->song_count; ?></div>
<div class="cel_year"><?php if ($libitem->year > 0) echo $libitem->year; ?></div>
<?php if (AmpConfig::get('show_played_times')) { ?>
<div class="cel_counter"><?php echo $libitem->object_cnt; ?></div>
<?php } ?>
<div class="cel_tags"><?php echo $libitem->f_tags; ?></div>
<?php if (User::is_registered()) { ?>
    <?php if (AmpConfig::get('ratings')) { ?>
    <div class="cel_rating" id="rating_<?php echo $libitem->id; ?>_album"><?php Rating::show($libitem->id, 'album'); ?></div>
    <?php } ?>
    <?php if (AmpConfig::get('userflags')) { ?>
    <div class="cel_userflag" id="userflag_<?php echo $libitem->id; ?>_album"><?php Userflag::show($libitem->id, 'album'); ?></div>
    <?php } ?>
<?php } ?>
<div class="cel_action">
    ...
    <?php if (Access::check('interface','50') && (!$libitem->allow_group_disks || ($libitem->allow_group_disks && !count($libitem->album_suite)))) { ?>
        <a id="<?php echo 'edit_album_'.$libitem->id ?>" onclick="showEdidivialog('album_row', '<?php echo $libitem->id ?>', '<?php echo 'edit_album_'.$libitem->id ?>', '<?php echo T_('Album edit') ?>', 'album_')">
